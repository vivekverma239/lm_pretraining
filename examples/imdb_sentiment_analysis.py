...
from tensorflow.keras.layers import Input, Dense, Dropout, CuDNNLSTM, Lambda
...
def process_data_for_classification(imdb_path, val_sample=5000 , tokenizer='nltk', maxlen=500):
    """
    ...
    """
    file_name = 'imdb_processed_data_maxlen_{}_tokenizer_{}_val_{}.pkl'.format(maxlen, tokenizer,val_sample)
    if not os.path.exists(file_name):
        ...
        pickle.dump([x_train, y_train , x_val , y_val, x_test, y_test], open(file_name, 'wb'))
        return x_train, y_train , x_val , y_val, x_test, y_test
    else:
        x_train, y_train , x_val , y_val, x_test, y_test = pickle.load(open(file_name, 'rb'))
        return x_train, y_train , x_val , y_val, x_test, y_test
